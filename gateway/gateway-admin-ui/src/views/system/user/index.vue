		...
		<el-container>
				...
				<el-main class="nopadding">
					<scTable ref="table" :apiObj="apiObj" @selection-change="selectionChange" stripe remoteSort remoteFilter>
						...
						<el-table-column label="ID" prop="userId" width="80" sortable='custom'></el-table-column>
						...
						<el-table-column label="所属部门" prop="deptName" width="150" sortable='custom'></el-table-column>
						<el-table-column align="center" label="所属角色" >
							<template #default="scope">
								<el-tag size="small" v-for="(data,index) in scope.row.roleList" :key="index" type="success">
									{{ data.roleName }}
								</el-tag>
							</template>
						</el-table-column>
						<el-table-column align="center" label="状态">
							<template #default="scope">
								<el-tag size="small" :type="scope.row.lockFlag === '0' ? 'success' : 'danger'">
									{{ scope.row.lockFlag === "0" ? "正常" : "禁用" }}
								</el-tag>
							</template>
						</el-table-column>
						<el-table-column label="创建时间" prop="createTime" width="170" sortable='custom'></el-table-column>
						<el-table-column label="更新时间" prop="updateTime" width="170" sortable='custom'></el-table-column>
						<el-table-column label="上次登录时间" prop="lastLoginTime" width="170" sortable='custom'></el-table-column>
						<el-table-column label="上次登录IP" prop="lastLoginIp" width="170" sortable='custom'></el-table-column>
	...
	export default {
		...
		methods: {
			...
			async table_del(row, index){
				let res = await this.$API.system.user.delete.delete(row.userId);
				if(res.code === 200){
