...
import cn.cleanarch.dp.gateway.component.GroovyCache;
...
@Slf4j
@Service
public class DynamicGatewayGroovyService {
    ...
    public GroovyHandleData handle(ServerWebExchange exchange, GroovyHandleData handleData, HttpEnum httpEnum) throws Exception {
        ...
        List<Long> groovyScriptIds = GatewayRouteGroovyCache.get(route.getId());
    ...
    public void refresh(long ... ids){
        GatewayGroovyScriptDO script;
        for (long id : ids){
            script = gatewayGroovyScriptService.findById(id);
     ...
     * @param script
     */
    public void refreshGroovyScript(GatewayGroovyScriptDO script){
    ...
    public void groovySort(String routeId){
        List<Long> idList = gatewayGroovyScriptService.findIdList(routeId);
        if (CollectionUtils.isEmpty(idList)){
            return ;
        }
        List<Long> cacheIdList = GatewayRouteGroovyCache.get(routeId);
        ...
        //重建网关缓存的规则引擎动态脚本关系
        GatewayRouteGroovyCache.remove(routeId);
        GatewayRouteGroovyCache.put(routeId, newIdList);
    }
    ...
    private void removeCacheGroovy(String routeId, long groovyScriptId){
        ...
        //移除网关与规则引擎动态脚本关系
        if (routeId != null){
            GatewayRouteGroovyCache.removeValue(routeId, groovyScriptId);
     ...
     * @param md5
     */
    public void instance(GatewayGroovyScriptDO script, String md5){
        boolean isNews = true;
        //量否缓存过
     ...
     * @param isNew
     */
    public void instance(GatewayGroovyScriptDO script, String md5, boolean isNew){
     ...
     * @param md5
     */
    public void instance(GatewayGroovyScriptDO script, String md5, boolean isNews, GroovyCache.GroovyServiceData groovyServiceData){
        //编译
        BaseGroovyService groovyService;
        try {
            groovyService = (BaseGroovyService) gatewayGroovyScriptService.instance(script);
        }catch(Exception e){
            return;
        }
        ...
        SpringUtil.getApplicationContext().getAutowireCapableBeanFactory().autowireBean(groovyService);
        //本地内存缓存网关与脚本关系，用于过滤器加载指定路径网关规则引擎脚本进行验证
        GatewayRouteGroovyCache.put(script.getRouteId(), script.getId());
