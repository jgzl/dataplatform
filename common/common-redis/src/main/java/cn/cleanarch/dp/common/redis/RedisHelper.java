...
import cn.cleanarch.dp.common.redis.convert.DateDeserializer;
import cn.cleanarch.dp.common.redis.convert.DateSerializer;
...
//        logger.warn("Use default RedisHelper, you'd better use a DynamicRedisHelper instead.");
     ...
     * @return
     */
    public <T> T hshGet(String key, String hashKey, TypeReference<T> typeReferenceRef) {
        return this.fromJson(hashOpr.get(key, hashKey),typeReferenceRef);
    }
    ...
    public <T> T fromJson(String json, TypeReference<T> valueTypeRef) {
        if (StrUtil.isBlank(json) || valueTypeRef == null) {
            return null;
        }
        try {
    ...
    /**
     * 将对象直接以json数据反序列化对象获取
     *
     * @param key 键
     * @author yunxiang.zhou01@hand-china.com 2018-06-11 13:46
     */
    public <T> T objectGet(String key, TypeReference<T> valueTypeRef) {
        String value = this.strGet(key);
        return this.fromJson(value, valueTypeRef);
    }
