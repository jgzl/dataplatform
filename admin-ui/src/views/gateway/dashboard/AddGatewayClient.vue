<template>
	<div>
		...
		<el-dialog title="添加客户端" v-model="dialogFormVisible" width="40%" :close-on-click-modal="false">
			<el-table size="small" :data="clientTableData" style="width: 100%">
				...
				<el-table-column label="操作" width="60">
					<template v-slot:default="scope">
						<el-button size="small" circle icon="el-icon-plus" type="success" title="添加" @click="handleAddRegClient(scope.row)"></el-button>
					</template>
				</el-table-column>
			</el-table>
				...
				<el-card class="box-card">
					<div slot="header" class="clearfix">
						...
						<span style="margin-left: 50px;">
							...
							<span style="font-size: 11pt;">
								...
								<el-tag size="small" type="warning" style="font-weight: bold;">{{form.id}}</el-tag> -
								<el-tag size="small" type="success" style="font-weight: bold;">{{form.uri}}</el-tag>
							</span>
						</span>
					...
					<el-table size="small" :data="tableData" style="width: 100%">
						...
						<el-table-column label="TOKEN" >
							<template v-slot:default="scope">
								...
								<el-popover trigger="click" placement="bottom" :v-model="true">
									...
									<el-button slot="reference" text @click="queryRegClientTokenInfo(scope.row, scope.$index)" title="点击查看JWT通行令牌"></el-button>
								</el-popover>
							</template>
						</el-table-column>
						...
						<el-table-column label="状态" prop="status" :formatter="formatterStatus">
							<template v-slot:default="scope">
								<div v-if="scope.row.regServerStatus==='0'"><i class="el-icon-success" style="color: #409EFF;"></i>&nbsp;<el-tag size="small">{{'允许通行'}}</el-tag></div>
								<div v-if="scope.row.regServerStatus==='1'"><i class="el-icon-error" style="color: #f00000;"></i>&nbsp;<el-tag size="small" type="danger">{{'禁止通行'}}</el-tag></div>
							</template>
						</el-table-column>
						...
						<el-table-column label="操作" width="100">
							...
							<el-dropdown trigger="click" @command="handleCommandRegClient">
								<el-button size="small" circle icon="el-icon-setting" title="设置" style="border: 0px;"></el-button>
								<template #dropdown>
									<el-dropdown-menu>
