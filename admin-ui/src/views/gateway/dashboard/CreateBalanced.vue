	...
	<div>
		...
		<el-card class="box-card" style="margin-top: 20px;">
			<div slot="header" class="clearfix">
				...
				<el-popover trigger="click" placement="bottom">
					<div style="font-size: 10pt;">
						...
						<span>3. 路径/proxy/为默认根路径，不可更改，只做网关路由规则识别，用于区分负载路由路径与普通路由路径。</span><br/>
						<span>4. 实际转发到服务端不包含该/proxy/根路径。</span><br/>
			...
			<el-dialog title="添加服务端" v-model="dialogFormVisible" width="40%" :close-on-click-modal="false">
				<el-table size="small" :data="routeTableData" style="width: 100%">
					...
					<el-table-column label="操作" width="60">
						<template v-slot:default="scope">
							<el-button v-if="scope.row.selected !== true" size="small" circle icon="el-icon-plus" type="success" title="点击添加" @click="handleAddRegServer(scope.row)"></el-button>
							<el-button v-if="scope.row.selected === true" size="small" circle icon="el-icon-check" type="primary" title="已添加" @click="handleAddRegServer(scope.row)"></el-button>
						</template>
					</el-table-column>
				</el-table>
				...
				<el-col :span="16">
					<el-card class="box-card">
						<el-table size="small" :data="tableData" style="width: 100%">
						  ...
						  <el-table-column label="操作" width="60">
						  	<template v-slot:default="scope">
						  		<el-button size="small" circle icon="el-icon-delete" title="删除" style="border: 0px; font-size: 12pt;" @click="handleDeleteServer(scope.row)"></el-button>
						  	</template>
						  </el-table-column>
						</el-table>
					</el-card>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goBack() {
				this.$router.push({ path: '/gateway/dashboard/loadBalanced', query: {} });
			},
