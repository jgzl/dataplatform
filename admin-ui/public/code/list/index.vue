			...
			<scTable ref="table" :apiObj="apiObj" row-key="<%= base.rowKey %>" @selection-change="selectionChange">
				...
				<el-table-column label="操作" fixed="right" align="right" width="140">
					<template #default="scope">
						<el-button text size="small" @click="table_show(scope.row, scope.$index)">查看</el-button>
						<el-button text size="small" @click="table_edit(scope.row, scope.$index)">编辑</el-button>
...
export default {
		...
		methods: {
			...
			async table_del(row, index){
				...
				let res = await this.$API.<%= api.del %>.post(reqData);
				if(res.code === 200){
			...
			saveForm(){
				this.$refs.saveDialog.submit(async (formData) => {
					this.isSaving = true;
					let res = await this.$API.<%= api.save %>.post(formData);
					this.isSaving = false;
					if(res.code === 200){
						//这里选择刷新整个表格 OR 插入/编辑现有表格数据
